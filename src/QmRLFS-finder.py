#!/usr/bin/env python
# Script Name: QmRLFS-finder.py (version v1.5)
...
# e-mail:      piroonj@bii.a-star.edu.sg
# Update:      22-Jan-2016
################################
##History
## QmRLFS-finder v1.5
...
class GenerateWindows():
    ...
    def __iter__(self): # must return an iterator!
        return iter(xrange(0,self.length,self.step))

prog_name = os.path.basename(sys.argv[0])
version = "v1.5"
today = datetime.datetime.today()
parser = argparse.ArgumentParser(prog="python %s"%prog_name, \
                                formatter_class=argparse.RawDescriptionHelpFormatter,\
                                description=textwrap.dedent('''\
                                    Description: 
                                     %s (version %s) is program to predict R-loop 
                                     forming regions in any DNA sequences'''%(prog_name, version)), \
...
if args.outfile == "":
    ...
    if not args.table and not args.fa and not args.bed and not args.custom_track:
        table_out = True
    if args.table or table_out:
        outfile_table = open("%s.out.table.txt"%args.outfile,"wb")
    if args.fa:
        outfile_fa = "%s.out.fa"%args.outfile
    if args.bed:
        outfile_bed = open("%s.out.bed"%args.outfile,"wb")
    if args.custom_track:
        outfile_custom_track = open("%s.out.customTrack.bed"%args.outfile,"wb")
...
if len(unknown_models)>0:
    exit('\n argument -m or --model does not support "%s"'%(",".join(list(unknown_models))))
...
def rez_search(riz_end, seq, seqlength): 
    ...
    for i in xrange(params['num_linker']):
        ...
        if seq_frag:
            ...
            for j in xrange(i + params['window_step'], i + rez_loop_search + 1):
                seq_tmp = seq[i:j]
...
def output_rlfs(riz, rez, seq, strand, desc, model):
    ...
    if args.coordinate_name or args.coordinate:
        ...
        if end_i < start_i:
            ...
            exit("In --coordinate chr:start-end and --coordinate_name, end-start+1 (%s-%s+1=%s) must equal to input sequence length (%s)\nexclude --coordinate_base_zero option may solve the problem."%(end_i,start_i,end_i-start_i+1,seqlength))
        elif seqlength != end_i-start_i+1:
            exit("\nIn --coordinate chr:start-end and --coordinate_name, end-start+1 (%s-%s+1=%s) must equal to input sequence length (%s)\ninclude --coordinate_base_zero option may solve the problem."%(end_i,start_i,end_i-start_i+1,seqlength))
        start_i = start_i-1
    else:
        chrom_i = desc
        start_i = 0
    ...
    if strand == "+":
        ...
        blockSize = "%s,%s"%(riz['length'],rez['length'])
        blockStarts = "%s,%s"%(0,int(rez['start'])-int(riz['start']))
    else:
        start = start_i + int(rez['start'])
        end = start_i + int(riz['end'])
        blockSize = "%s,%s"%(rez['length'],riz['length'])
        blockStarts = "%s,%s"%(0,int(riz['start'])-int(rez['start']))
        
    if args.keep_name:
        name = desc
    else:
        name = "%s.%s.%s"%(chrom_i,end-start,start)
        
    if args.coordinate_name or args.coordinate:
        location = "%s:%s-%s"%(chrom_i,start+1,end)
        fasta_comment = "%s.%s (%s:%s-%s) (model=%s;strand=%s)" % (name,riz['model'],chrom_i,start+1,end, model, strand)
    else:
        location = "%s:%s-%s"%(desc,start+1,end)
        fasta_comment = "%s.%s (model=%s;strand=%s)" % (name,riz['model'], model, strand)
    ...
    if args.bed:
        ...
        outfile_bed.write("%s\n"%"\t".join(map(str, [chrom_i, start, end, "%s.%s"%(name,riz['model']), score, strand, start, end, itemRgb, blockCount, blockSize, blockStarts])))
    if args.custom_track:
        name = name.replace(" ",".")
        chrom_i = chrom_i.replace(" ",".")
        outfile_custom_track.write("%s\n"%"\t".join(map(str, [chrom_i, start, end, "%s.%s"%(name,riz['model']), score, strand, start, end, itemRgb, blockCount, blockSize, blockStarts])))

    if args.table or table_out:
            outfile_table.write("%s\n"%("\t".join(map(str, [desc, riz['model'], location, riz["start"], riz["end"], riz["length"],
    ...
    if riz_list:  
	    ...
	    ## find REZ for each riz
            ...
            if rez:
                ...
                if head:
                    if args.table or table_out:
                        outfile_table.write("%s\n"%"\t".join(tableheaders_combined))

                    if args.custom_track:
                        outfile_custom_track.write('track name="%s" description="%s" visibility=4 colorByStrand="5,190,255 244,0,224"\n'%(args.track_name, args.track_description))
                    head = False
...
def main():
    ...
    if args.outfile != "":
        if args.log:
            outlog = open("%s.log"%args.outfile, "wb")
        if args.log:
            outlog.write( "%s (version %s)\n %s %s\n" %( prog_name, version, "run on", today.strftime(dateformat) ) )
            outlog.write( " command line: python %s\n"%" ".join(sys.argv) )
        else:
            print "%s (version %s)\n %s %s" %( prog_name, version, "run on", today.strftime(dateformat) )
            print " command line: python %s"%" ".join(sys.argv)
    ...
    for record in SeqIO.parse(infile, "fasta"):
        if args.outfile != "" and args.verbose:
            if args.log:
                ...
                print "\n%sSearch RLFS in %s"%(">"*2,record.description)
        ...
        for model in params["regex_models"]:
            ...
            if args.outfile != "" and args.verbose:
                if args.log:
                    outlog.write( "\n%sModel: %s\n\n"%(" "*4,model) )
                    outlog.write( "%s- Search RLFS in positive strand\n\n%s> Process\n"%(" "*4," "*6) )
                    outlog.write( "%s#--1--2--3--4--5--6--7--8--9--# %%\n"%(" "*8) )
                    outlog.write( "%s#"%(" "*8))
                    outlog.flush()
                else:
                    print "\n%sModel: %s\n"%(" "*4,model)
                    print "%s- Search RLFS in positive strand\n\n%s> Process"%(" "*4," "*6)
                    print "%s#--1--2--3--4--5--6--7--8--9--# %%"%(" "*8)
            ...
            if args.outfile != "" and args.verbose:
                if args.log:
                    outlog.write( "%s- Search RLFS in negative strand\n\n%s> Process\n"%(" "*4," "*6) )
                    outlog.write( "%s#--1--2--3--4--5--6--7--8--9--# %%\n"%(" "*8) )
                    outlog.write( "%s#"%(" "*8) ) 
                    outlog.flush()
                else:
                    print "%s- Search RLFS in negative strand\n\n%s> Process"%(" "*4," "*6)
                    print "%s#--1--2--3--4--5--6--7--8--9--# %%"%(" "*8)
                    sys.stdout.write( "%s#"%(" "*8) )
            #
            if genomic_strand == '-':
